...
#   ClusterSim_1000_750_A_100_binomial_TRUE_neworder.RData
...
# Paths are relative to the outputs/ folder produced by the simulation jobs.
# Set sim_outputs_root to the folder that contains clustersims/ and RAW_HAZEL_BINOMIAL/.
# By default this assumes you run from submission_revision/code/simulation/3_tables/
# and that outputs/ sits two levels up at submission_revision/code/simulation/outputs/.
# Adjust if you placed the outputs folder elsewhere.
sim_outputs_root <- file.path("..", "..", "..", "outputs")

data_folder   <- file.path(sim_outputs_root, "clustersims", "A_100_binomial_hazel_table1")

# t=300 batch_0 contained 2 outlier replicas (reps 42 & 70, z2/cluster-3 RMSE ~251)
# caused by FPCA score extraction hitting max iterations. Use batch_1 (clean 100-rep
# rerun) directly instead of the combined file.
t300_override <- file.path(sim_outputs_root, "RAW_HAZEL_BINOMIAL", "A_100_binomial_hazel_table1", "batch_1")

t_lengths <- c(300, 750, 2000)

print(paste("Building MSE table for num_indvs =", num_indvs))
print(paste("Data folder:", data_folder))
print(paste("t=300 override folder:", t300_override))
...
load_est_values <- function(n, t) {
  folder <- if (t == 300) t300_override else data_folder
  fname <- paste("ClusterSim", n, t, scenario, num_replicas, est_choice, suffix, sep = "_")
  print(paste("Loading:", fname, "from", basename(folder)))
  e <- new.env()
  load(file.path(folder, fname), envir = e)
